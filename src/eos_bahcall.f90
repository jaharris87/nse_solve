...
! Calculate plasma quantities
  lambda0=sqrt(4*pi*rho*avn/abar)*(e2/bkt)**1.5 ! DGC, Eq. 3
  ae=(3./(4.*pi*avn*rho*ye))**onethird ! electron-sphere radius
  gammae=e2/(ae*bkt) ! electron Coulomb coupling parameter 
  zinter=zibar/(ztilde**.58*zbar**.28)
  If(idiag>2) Write(lun_diag,'(a14,9es12.5)') 'Bahcall SCRN', t9, rho, ye, z2bar, zbar, sratio, ztilde,ztilde*lambda0, gammae
...
&    +(0.1203*lefmkt**4)-(0.012857*lefmkt**5)-(0.014768*lefmkt**6)
  ...
  If(itemp>0.and.lefmkt>1.578347.and.lefmkt<1.5) Then
...
Subroutine y_moment(y,ztot,ytot,abar,zbar,z2bar,zibar)  
!------------------------------------------------------------------------------  
...
!------------------------------------------------------------------------------  
  ...
  Real(8), Intent(out) :: ztot,ytot,abar,zbar,z2bar,zibar
  Real(8)              :: atot
...
! Calculate abundance moments
  ...
  If(idiag>2) Write(lun_diag,'(a4,6es12.5)') 'YMom',ytot,abar,zbar,z2bar,zibar,ye
