...
! MESA values for constants
! Real(8), Parameter :: pi = 3.1415926535897932384d0
! Real(8), Parameter :: epmev = 1.602176487d-6
! Real(8), Parameter :: kerg = 1.3806504d-16
! Real(8), Parameter :: qe = 4.80320440d-10
! Real(8), Parameter :: mn = 1.6749286d-24
! Real(8), Parameter :: mp = 1.6726231d-24
! Real(8), Parameter :: me = 9.1093826d-28
! Real(8), Parameter :: avn = 6.02214179d23
! Real(8), Parameter :: clt = 2.99792458d10
! Real(8), Parameter :: h = 6.62606896d-27 / epmev
! Real(8), Parameter :: hbar = 0.5d0 * h / pi
! Real(8), Parameter :: amu = 1.0d0 / avn / epmev
! Real(8), Parameter :: bok = kerg * 1.0d9 / epmev
! Real(8), Parameter :: e2 = qe*qe / epmev
! Real(8), Parameter :: ele_en = me * clt * clt / epmev
! Real(8), Parameter :: mex_p = (mp/epmev - amu)*clt*clt
! Real(8), Parameter :: mex_n = (mn/epmev - amu)*clt*clt
...
!-----------------------------------------------------------------------------  
  ...
  Integer :: it9i(24)
  Character (LEN=5) nam                                 
! Real(8), Parameter :: mex_p=7.28897050, mex_n=8.07131710
  Real(8), Parameter :: mex_p=7.28897064, mex_n=8.07131728
...
!$OMP PARALLEL
  Allocate (gg(0:ny))
  If(itemp>0) Allocate (dlngdt9(0:ny))
!$OMP End PARALLEL
  nname(0)=' === '
  angm(0)=0.0
  Do l=1,ny                                                  
    ...
    zz(l)=real(iz(l))                                                
    nn(l)=real(in(l))                                                
    be(l)=mex_n*nn(l)+mex_p*zz(l)-mex(l)
    angm(l)=2.*sp+1.
    nname(l)=nam
  EndDo
...
Subroutine index_pp
  ...
  Do l=1,ny                                                  
    If(nint(zz(l))== 0 .and. nint(aa(l))== 1) i_nn  =l
    If(nint(zz(l))== 1 .and. nint(aa(l))== 1) i_pp  =l
    If(nint(zz(l))== 2 .and. nint(aa(l))== 4) i_he4 =l
    If(nint(zz(l))== 6 .and. nint(aa(l))==12) i_c12 =l
    If(nint(zz(l))== 8 .and. nint(aa(l))==16) i_o16 =l
    If(nint(zz(l))==10 .and. nint(aa(l))==20) i_ne20=l
    If(nint(zz(l))==12 .and. nint(aa(l))==24) i_mg24=l
    If(nint(zz(l))==14 .and. nint(aa(l))==28) i_si28=l
    If(nint(zz(l))==16 .and. nint(aa(l))==32) i_s32 =l
    If(nint(zz(l))==18 .and. nint(aa(l))==36) i_ar36=l
    If(nint(zz(l))==20 .and. nint(aa(l))==40) i_ca40=l
    If(nint(zz(l))==22 .and. nint(aa(l))==44) i_ti44=l
    If(nint(zz(l))==24 .and. nint(aa(l))==48) i_cr48=l
    If(nint(zz(l))==26 .and. nint(aa(l))==52) i_fe52=l
    If(nint(zz(l))==28 .and. nint(aa(l))==56) i_ni56=l
    If(nint(zz(l))==30 .and. nint(aa(l))==60) i_zn60=l

    If(nint(zz(l))==26 .and. nint(aa(l))==54) i_fe54=l
    If(nint(zz(l))==26 .and. nint(aa(l))==56) i_fe56=l
    If(nint(zz(l))==26 .and. nint(aa(l))==58) i_fe58=l
    If(nint(zz(l))==28 .and. nint(aa(l))==58) i_ni58=l
  EndDo
  i_alpnp=(/i_nn,i_pp,i_he4,i_c12,i_o16,i_ne20,i_mg24,i_si28,i_s32,&
  &         i_ar36,i_ca40,i_ti44,i_cr48,i_fe52,i_ni56,i_zn60,i_fe56/)

  Return
End Subroutine index_pp
...
!$OMP PARALLEL      
  ...
  If(itemp>0) Allocate (dcsect1dt9(nr),dlnrpf1dt9(nr),dh1dt9(nr))
!$OMP End PARALLEL
  Do j=1,nr
! Read REACLIB data
...
!$OMP PARALLEL      
  ...
  If(itemp>0) Allocate (dcsect2dt9(nr),dlnrpf2dt9(nr),dh2dt9(nr))
!$OMP End PARALLEL
  Do j=1,nr
...
!$OMP PARALLEL      
  ...
  If(itemp>0) Allocate (dcsect3dt9(nr),dh3dt9(nr))
!$OMP End PARALLEL
  Do j=1,nr
...
!$OMP PARALLEL
  Allocate (b1(nan(1)))
  If(itemp>0) Allocate (dr1dt9(nan(1)))
!$OMP End PARALLEL
  Do j=1,nan(1)
    Read( 2)  a1(j),mu1(j) 
    n11(j)=n1i(1,mu1(j))
...
!$OMP PARALLEL
  Allocate (b2(nan(2)))
  If(itemp>0) Allocate (dr2dt9(nan(2)))
!$OMP End PARALLEL
  Do j=1,nan(2)
    Read( 2)  a2(j),mu2(j)  
    n21(j)=n2i(1,mu2(j))
...
!$OMP PARALLEL
  Allocate (b3(nan(3)))
  If(itemp>0) Allocate (dr3dt9(nan(3)))
!$OMP End PARALLEL
