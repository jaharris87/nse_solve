...
Subroutine eos_cv(rho,t9,y,cv)
  ...
  Real(8) :: ye,atot,ztot,ytot,abar,zbar,z2bar,zibar
...
! Calculate Ye and other needed moments of the abundance distribution
    call y_moment(y,atot,ztot,ytot,abar,zbar,z2bar,zibar)  
    ye=zbar/abar
...
!-----------------------------------------------------------------------
  Use constants
  Use controls
  Use nse_module, only: yense
  include 'vector_eos.dek'
  Real(8), Intent(in) :: t9, rho, y(*)
  Real(8), Intent(out) :: ye, ztilde, zinter, lambda0, gammae, dztildedt9
  Real(8) :: ztot,ytot,bkt,abar,zbar,z2bar,zibar,atot
...
! Calculate plasma quantities
  ...
  If(idiag>2) Write(lun_diag,'(a14,9es12.5)') 'Helmholtz EOS', t9, rho, ye,z2bar,zbar,sratio, ztilde, ztilde*lambda0, gammae
...
Subroutine y_moment(y,atot,ztot,ytot,abar,zbar,z2bar,zibar)  
!------------------------------------------------------------------------------  
...
!------------------------------------------------------------------------------  
  ...
  Real(8), Intent(out) :: atot,ztot,ytot,abar,zbar,z2bar,zibar
! Real(8)              :: atot
...
! Calculate abundance moments
  ...
  If(idiag>2) Write(lun_diag,'(a4,7es12.5)') 'YMom',ytot,abar,zbar,z2bar,zibar,atot,ztot
